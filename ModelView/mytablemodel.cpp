...
QVariant MyTableModel::data(const QModelIndex &index, int role) const
{
    ...
    if (index.row()*m_columnNum + index.column() < m_data.count())
    {
        if (role == Qt::DisplayRole || role == Qt::EditRole) {
            ...
            return m_data[index.row()*m_columnNum + index.column()]->toolTip;//数据的提示信息
        }
        else if (role == Qt::UserRole)
        {
            return QVariant::fromValue(m_data[index.row()*m_columnNum + index.column()]);
        }
    }
    return QVariant();
}
...
bool MyTableModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    if (index.row()*m_columnNum + index.column() < m_data.count())
    {
        ...
        {
            m_data[index.row()*m_columnNum + index.column()]->content = value.value<QString>();
